...
instance Pretty Expr where   
  ...
  pp p (Fork val) = brace p 100  ("fork " ++ pp 101 val)
  pp p (Acc val) = brace p 100 ("accept " ++ pp 101 val)
  pp p (Req val) = brace p 100 ("request " ++ pp 101 val)
  pp p (Send val val') = brace p 100 ("send " ++ pp 0 val ++ " on " ++ pp 101 val')
  pp p (Recv val) = brace p 100 ("receive " ++ pp 101 val)
  pp p (Sel la val) = brace p 100 ("select " ++ prettyLabel la  ++ " on " ++ pp 101 val)
  pp p (Case val ex ex') = brace p 1 ("case " ++ pp 0 val ++ " of { " ++ pp 0 ex ++ "; " ++ pp 0 ex' ++ " }")
  pp p (Close val) = brace p 100 ("close " ++ pp 101 val)
  pp p (New ty) = brace p 100  ("new " ++ pp 101 ty)
