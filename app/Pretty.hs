...
instance Pretty Program where
  pp i p (abs, cbs, es) = (if null abs then "" 
...
instance Pretty Type where
  ...
  pp i p (EAll s ki xs ty) = indent i $ brace p 5 ("∀(" ++ s ++ " : " ++ pp 0 0 ki ++ "). (" 
                                                   ++ pp 0 0 xs ++ ") => " ++ pp 0 5 ty)
  pp i p (EArr st1 ty1 [] st2 ty2) = indent i $ brace p (-1) (pp 0 0 st1 ++ "; " ++ pp 0 0 ty1 ++ " → " 
                                                              ++ pp 0 0 st2 ++ "; " ++ pp 0 0 ty2)
  pp i p (EArr st1 ty1 ctx st2 ty2) = indent i $ brace p (-1) (pp 0 0 st1 ++ "; " ++ pp 0 0 ty1 ++ " → " ++ "∃" 
                                                               ++ pp 0 0 ctx ++ ". " ++ pp 0 0 st2 ++ "; " 
                                                               ++ pp 0 0 ty2)
  ...
  pp i p (SSend s ki st ty ty2) = indent i $ brace p 100 ("!(∃" ++ s ++ " : " ++ pp 0 0 ki ++ ". " ++ pp 0 0 st 
                                                          ++ "). " ++ pp 0 100 ty2)
  pp i p (SRecv s ki st ty ty2) = indent i $ brace p 100 ("?(∃" ++ s ++ " : " ++ pp 0 0 ki ++ ". " ++ pp 0 0 st 
                                                          ++ "). " ++ pp 0 100 ty2)
...
instance Pretty Expr where
  ...
  pp i p (Case val ex ex') = indent i $ brace p 1 ("case " ++ pp 0 0 val ++ " of { " ++ pp 0 0 ex ++ "; " 
                                                   ++ pp 0 0 ex' ++ " }")
